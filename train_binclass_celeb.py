...
from architectures import ECBAM2
...
def main():
    ...
    parser.add_argument('--celeb_faces_df_path', type=str, action='store',
                        help='Path to the Pandas Dataframe obtained from extract_faces.py on the Celeb dataset. '
                             'Required for training/validating on the DFDC dataset.')
    parser.add_argument('--celeb_faces_dir', type=str, action='store',
                        help='Path to the directory containing the faces extracted from the Celeb dataset. '
                             'Required for training/validating on the DFDC dataset.')
    ...
    # Parse arguments
    net_class = getattr(ECBAM2, args.net)
    ...
    for dataset in train_datasets:
        ...
        elif dataset.split('-')[0] == 'celeb' and (celeb_df_path is None or celeb_faces_dir is None):
    ...
    splits = split.make_splits(
        dfdc_df=dfdc_df_path, ffpp_df=ffpp_df_path, celeb_df=celeb_df_path,
        dfdc_dir=dfdc_faces_dir, ffpp_dir=ffpp_faces_dir, celeb_dir=celeb_faces_dir,
