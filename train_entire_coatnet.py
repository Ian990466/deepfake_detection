...
from architectures.coatnetv2_test import coatnetT
...
def validation_routine(net, device, val_loader, criterion, tb, epoch, tag: str, loader_len_norm: int = None):
    net.eval()
    ...
    # Logging
    val_loss /= val_num
    tb.add_scalar('{}/loss'.format(tag), val_loss, epoch+1)
    ...
    if isinstance(criterion, nn.BCEWithLogitsLoss):
        ...
        tb.add_scalar('{}/roc_auc'.format(tag), val_roc_auc, epoch+1)
        tb.add_pr_curve('{}/pr'.format(tag), val_labels, val_pred, epoch+1)
...
def main():
    ...
    parse.add_argument('--valsamples', type=int, help='Limit the number of validation samples per epoch', default=-1)
    ...
    parse.add_argument('--log_dir', type=str, help='Directory for saving the training logs', default='runs/test2/')
    parse.add_argument('--models_dir', type=str, help='Directory for saving the models weights', default='./weights/CoatNet/binclass/')
    ...
    for epoch in range(n_epochs):
        ...
        tb.add_scalar('train/roc_auc', train_roc_auc, epoch+1)
        tb.add_pr_curve('train/pr', train_labels, train_pred, epoch+1)

        # Validation
        val_loss = validation_routine(model, device, val_loader, criterion, tb, epoch, 'val')
        tb.flush()
